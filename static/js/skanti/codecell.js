...
var skanti = (function (skanti) {
    ...
    var CodeCell = function (notebook) {
        ...
        skanti.Cell.apply(this, arguments);
    };


    CodeCell.prototype = new skanti.Cell();
    ...
    CodeCell.prototype.request_tooltip_after_time = function (pre_cursor,time){
        ...
        if (pre_cursor === "" || pre_cursor === "(" ) {
            ...
            that.tooltip_timeout = setTimeout(function(){
                    //skanti.notebook.request_tool_tip(that, pre_cursor)
                },time);
        }
    };
    ...
    CodeCell.prototype.handle_codemirror_keyevent = function (editor, event) {
        ...
        if (event.keyCode === 13 && (event.shiftKey || event.ctrlKey)) {
            ...
            if (pre_cursor.trim() === "") {
                ...
                skanti.notebook.complete_cell(this, line, cur.ch);
                return true;
            }
...
/* TODO:
            var msg_id = skanti.notebook.kernel.execute(name+"?");
            skanti.notebook.msg_cell_map[msg_id] = skanti.notebook.selected_cell().cell_id;
            that.remove_and_cancel_tooltip();
            setTimeout(function(){that.code_mirror.focus();}, 50);
*/
        });
    ...
    CodeCell.prototype.select = function () {
        skanti.Cell.prototype.select.apply(this);
    ...
    CodeCell.prototype.append_output = function (json) {
        this.collapse()
        this.set_input_prompt(json.prompt_number);
        var toinsert = this.create_output_area();
        toinsert.find('div.prompt').addClass('output_prompt').html('Out[' + json.prompt_number + ']:');
        var element = $("<div/>").addClass("box_flex1 output_subarea");
        var outputbox = $("<div/>").attr('id',this.cell_id)
        element.append(outputbox);
        toinsert.append(element);
				this.element.find('div.output').append(toinsert);
        this.append_mime_type(json, outputbox);
        // If we just output latex, typeset it.
        if ( (json.latex !== undefined) || (json.html !== undefined) ) {
            this.typeset();
        };
				if($("#"+this.cell_id).html() != '') { this.expand(); }
    };

    CodeCell.prototype.append_mime_type = function (json, element) {
        if (json.html !== undefined) {
            this.append_html(json.content, element);
        } else if (json.latex !== undefined) {
            this.append_latex(json.content, element);
        } else if (json.svg !== undefined) {
            this.append_svg(json.content, element);
        } else if (json.png !== undefined) {
            this.append_png(json.content, element);
        } else if (json.jpeg !== undefined) {
            this.append_jpeg(json.content, element);
        } else if (json.javascript !== undefined) {
            this.append_javascript(json.content,element)
        } else if (json.text !== undefined) {
            this.append_text(json.content, element);
        };
    };

    CodeCell.prototype.getCell = function() {
        return $("#"+this.cell_id);
    }

		CodeCell.prototype.load_library = function(url) {
			$('body').append($("<script/>").attr('type','text/javascript').attr('src',url));
		}

    CodeCell.prototype.load_css = function(url) {
      $('body').append($("<style/>").attr('type','text/css').attr('src',url));
    }
    ...
    CodeCell.prototype.append_javascript = function (javascript, element) {
        skanti.runtime.execute(javascript,this)
    };
